...
from Custom_nk.return_gradients_driver import GradVMC
...
parser.add_argument("--arch", default="TFIM", type=str)
parser.add_argument(
    "--arch_params",
    default='{"J": 1, "h": 0.5}',
    type=dict_or_none,
...
grad_folder = f"{PROJECT_ROOT}Data/Gradients/MC/"
gh = GradHandler(grad_folder, s.name, model.label, args.L, args.PBC, args.n_dim, **s.H_par)
...
grads_flat = gs.run(n_iter=args.M)

if g.n_nodes < 21: 
    E_gs = s.ground_state
    print("Final relative error: ", RE(gs.energy.mean, E_gs))

gh.save_gradients_flattened(grads_flat)