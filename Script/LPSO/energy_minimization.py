...
parser.add_argument("--sampling_dtype", default=None, type=jax_dtype)
...
parser.add_argument("--compute_sigma", default=False, type=str2bool)
...
if args.sampling_dtype.label is not None:
    _folder = f"LPS/{args.sampling_dtype.label}/"
else:
    _folder = ""
...
def linear_diag_shift(step, steps, start, finish):
    return start + step / steps * (finish - start)
diag_shift = lambda x: linear_diag_shift(x, args.M, 1e-3, 1e-6)
...
l, t = gs.run(n_iter=args.M, out=log, timeit=args.timeit, compute_sigma=args.compute_sigma, 
              compute_gradient_error=args.sampling_dtype.label is None)
...
if args.save_history:
    hist_folder = f"{PROJECT_ROOT}Data/History{f'_{solver_name}' if solver_name else ''}/{_folder}MC/"
