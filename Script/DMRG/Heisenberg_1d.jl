...
function main()
    if length(ARGS) < 2
        println("Usage: julia Heisenberg_1dim.jl <L> <J>")
        exit(1)
    end
    ...
    for j in 1:L
        jp = j % L + 1  
        H_operator += J,sigx,j,sigx,jp
        H_operator += J,sigy,j,sigy,jp
    ...
    new_result = DataFrame(L = [L], J = [J], energy = [energy])

    file_path = "../Data/DMRG/Energy/Heisenberg_1dim.csv"
