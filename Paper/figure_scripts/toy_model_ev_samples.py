...
def bound_KL(sigma: float) -> float:
    # Formula: exp(0.5 * (sigma*t)^2) * erfc( (sigma*t) / sqrt(2) )
    # return 2*(1-math.exp(4*sigma ** 2) * math.erfc(2*sigma))
    return sigma
...
def expect(obs, logpsi, x):
    ...
    mean = jnp.mean(local_kernel)
    var = jnp.mean(jnp.abs(local_kernel - mean) ** 2.0)
    return mean, var
...
def get_evs(n, seed, sigma, obs_name):
    ...
    # Freeze back into a FrozenDict
    mc_state_noisy.parameters = flax.traverse_util.unflatten_dict(params_noisy, sep='/')
    # state_ref = mc_state_ref.to_array(normalize=True)
    # state_noisy = mc_state_noisy.to_array(normalize=True)
    if obs_name == "X":
        ps = ["I"] * n
        ps[0] = "X"
        ps = "".join(ps)
    ...
    # log_psis_fn_noisy = jax.vmap(lambda _x: mc_state_noisy._apply_fun({"params": mc_state_noisy.parameters}, _x))
    # Thermalize
    diffs = []
    stds = []
    ...
    for t_step in therm_steps:
        ...
        # m_ref, v_ref = expect(observable, log_psis_fn_ref, samples_ref)
        # m_noisy, v_noisy = expect(observable, log_psis_fn_ref, samples_noisy)
    #     var_tot = v_ref + v_noisy
    #     diff = jnp.abs(m_ref - m_noisy)
    #     diffs.append(diff)
    #     stds.append(jnp.sqrt(var_tot / n_therm))
    #     print(jnp.sqrt(var_tot / n_therm))
    # plt.plot(therm_steps, diffs)
    # plt.plot(therm_steps, stds)
    # plt.yscale("log")
    # plt.show()
    # Sample
...
def main(n):
    ...
    # fig_accept, axs_accept = plt.subplots(1, 1)
    # axs= [axs_bias, axs_accept]
    axs = [axs_bias, ]
    fig.set_size_inches(6, 6)
    # fig_accept.set_size_inches(6, 6)
    ...
    for obs_name in obs_names:
        ...
        for j, seed in enumerate(seeds):
            ...
            # axs_accept.plot(sigma_list, variances[0, :, j], label=r"Ref" if j == (n_seeds - 1) else None,
            #             marker='.', markersize=10, linestyle='', alpha=alpha_spacing[j], color=reds(cmap_val))
            # axs_accept.plot(sigma_list, variances[1, :, j], label=r"Noisy" if j == (n_seeds - 1) else None,
            #             marker='.', markersize=10, linestyle='', alpha=alpha_spacing[j], color=blues(cmap_val))
            # axs_accept.plot(sigma_list, acceptances[0, :, j], label=r"Ref" if j == (n_seeds - 1) else None,
            #             marker='.', markersize=10, linestyle='', alpha=alpha_spacing[j], color=reds(cmap_val))
            # axs_accept.plot(sigma_list, acceptances[1, :, j], label=r"Noisy" if j == (n_seeds - 1) else None,
            #             marker='.', markersize=10, linestyle='', alpha=alpha_spacing[j], color=blues(cmap_val))
            # 2 sigma sampling error
    ...
    # axs_accept.legend()
    # axs_accept.set_ylabel(r'$\mathrm{Var}[f(x)]$')
    # axs_accept.set_yscale('log')
    # axs_accept.legend()
    # axs_accept.set_ylabel("Acceptance rate")
    ...
    # fig.suptitle(rf"$n={n}$ with {n_seeds} seeds")
    fig.tight_layout()
    fig.savefig(f"./figures/ev_n_{n}.pdf")
    plt.show()
    # fig_accept.suptitle(rf"$n={n}$ with {n_seeds} seeds")
    # fig_accept.tight_layout()
    # fig_accept.savefig(f"./figures/ev_n_{n}_{obs_name}_accept.pdf")
