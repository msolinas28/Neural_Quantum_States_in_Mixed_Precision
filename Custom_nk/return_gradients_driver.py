    ...
    def run(
            ...
            with timing.timed_scope(force=timeit) as timer:
                with tqdm(
                    ...
                    for step in self.iter(n_iter, step_size):
                        ...
                        grad_flat, _ = ravel_pytree(self._loss_grad)
                        grads_flat.append(grad_flat)
            ...
            if timeit:
                ...
                if self._is_root:
                    print(timer)
                    if out == ():
                        return timer, np.array(grads_flat).flatten()
                    else:
                        return loggers[0], timer, np.array(grads_flat).flatten()
        
            if out != ():
                return loggers[0], np.array(grads_flat).flatten()
            else:
                return np.array(grads_flat).flatten()
            
class LPEGradVMC(LPE_VMC):
    def run(
            self,
            n_iter: int,
            out: AbstractLog | Iterable[AbstractLog] | str | None = (),
